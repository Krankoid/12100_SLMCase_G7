...
# RUN_TAG is appended to output filenames so scenarios don't overwrite each other.
# CodeCarbon v3+ auto-detects country via IP geolocation (no manual code needed).
...
def main():
    ...
    print(f"[CodeCarbon] tag={RUN_TAG} device={DEVICE} "
          f"duration={dt:.1f}s energy={energy_kwh:.6f} kWh emissions={emissions_kg:.6f} kgCO2e")
